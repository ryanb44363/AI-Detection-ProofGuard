...
export default function UploadForm() {
    ...
    if (isImage || isPdf || isTextLike) {
      try {
        if (isTextLike) {
          ...
          // Convert HEIC/HEIF to JPEG client-side for preview using heic2any
          try {
            const mod = await import('heic2any');
            const heic2any = (mod as any).default || (mod as any);
            const converted = await heic2any({ blob: file, toType: 'image/jpeg', quality: 0.92 });
            const outBlob: Blob = Array.isArray(converted) ? converted[0] : converted;
            ...
            // If conversion fails, leave preview null; analysis still proceeds
            previewDataUrl = null;
          }
        } else {
          previewDataUrl = await new Promise<string>((resolve, reject) => {
    ...
    try {
      ...
      try {
        ...
        navigateToResult({
          ...
          isImage,
          isPdf,
          result: localRes as any,
        });
      } catch {
        // If even fallback fails, navigate to result page with error payload
        navigateToResult({
          fileName: file.name,
          previewUrl: isImage || isPdf || isTextLike ? previewDataUrl : null,
          isImage,
  ...
  // buildResultHtml no longer used; navigation uses /result.html?id=... with localStorage payload.
